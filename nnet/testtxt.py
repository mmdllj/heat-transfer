...
W31 = tf.Variable(tf.truncated_normal([2000, 2000], stddev=0.1))
W32 = tf.Variable(tf.truncated_normal([2000, 2000], stddev=0.1))
b3 = tf.Variable(tf.zeros([2000]) + 0.1)
L3 = tf.matmul(tf.square(L2), W32) + tf.matmul(L2, W31) + b3

W4=tf.Variable(tf.truncated_normal([2000,1], stddev=0.1))
...
for __epo in range(10):
    ...
    #print("%d,%0.4f, %0.4f" % (__epo, train_acc, 1-train_loss), file=f)
    f.write("%d,%0.4f, %0.4f\n" % (__epo, train_acc, 1-train_loss))
f.close()
saver=tf.train.Saver()
save_path=saver.save(sess, "model/tfmodel_chapter4")
...
f.write('测试数据平均准确率：'+str(1-sess.run(loss, feed_dict={X: xTest, Y: yTest})))#0.8945742920041084
f.close()
