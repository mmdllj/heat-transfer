...
hidden1num=1000
W11 = tf.Variable(tf.truncated_normal([8, hidden1num], stddev=0.1))
# W12 = tf.Variable(tf.truncated_normal([8,1000],stddev = 0.1))
b1 = tf.Variable(tf.zeros([hidden1num]) + 0.1)
L1 = tf.matmul(X, W11) + b1
# L1 = tf.matmul(tf.square(X),W12)+tf.matmul(X,W11)+b1
# L1_drop = tf.nn.dropout(L1,keep_prob)

hidden2num=2000
W21 = tf.Variable(tf.truncated_normal([hidden1num, hidden2num], stddev=0.1))
W22 = tf.Variable(tf.truncated_normal([hidden1num, hidden2num], stddev=0.1))
b2 = tf.Variable(tf.zeros([hidden2num]) + 0.1)
...
f.write('测试数据平均准确率：', 1-sess.run(loss, feed_dict={X: xTest, Y: yTest}))
f.close()
